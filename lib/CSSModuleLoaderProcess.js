...
export class CSSModuleLoaderProcess {
  ...
  locate (load) {
    load.metadata.source = !!getParameterByName('source', load.name)
  ...
  fetch (load, systemFetch) {
    ...
    return systemFetch(load)
      ...
      .then(({ injectableSource, exportTokens }) => {
        ...
        styleSheet.exportedSource = this._sourceExportModule(injectableSource)
        return styleSheet
      })
  }
  ...
  _sourceExportModule (injectableSource) {
    return `module.exports = ${JSON.stringify(injectableSource)}`
  }
