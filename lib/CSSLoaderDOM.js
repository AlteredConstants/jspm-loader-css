...
export class CSSLoaderDOM extends CSSModuleLoaderProcess {
  constructor (plugins, moduleName) {
    ...
    this._debouncedSortingAndInject = (styleSheet) => {
      debounce(this._sortAndInjectInDom, 500).call(this, styleSheet)
      return styleSheet
    }
  ...
  fetch (load, systemFetch) {
    const fetchSheet = super.fetch(load, systemFetch)
    if (load.metadata.source) {
      return fetchSheet.then(styleSheet => styleSheet.exportedSource)
    } else {
      return fetchSheet
